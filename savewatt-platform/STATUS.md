...
- **Internal admins**: `julia@savewatt.fr` and `gary.abitbol@gmail.com` whitelisted as `SUPER_ADMIN` (org `org_savewatt`) in remote + local D1; matching Clerk **production** users created with temporary passwords (kept outside the repo). They sign in on `admin.savewatt.fr`.
- **Clerk production is live** (platform version `264b0c10`, then `8e6a2d46`): `pk_live`/`sk_live` in the build and Worker secrets; signed-out navigations redirect to sign-in. Google sign-in enabled with SaveWatt's own OAuth client. OAuth **dynamic client registration** enabled on the instance (used by MCP clients).
- **MCP server** (`savewatt-mcp`, `https://mcp.savewatt.fr/mcp`, version `581ebe71`): OAuth via Clerk (protected-resource metadata + `WWW-Authenticate`, Clerk JWT verified against the instance JWKS; internal whitelist provisioned on first sign-in; only `SUPER_ADMIN`/`OPERATOR_FINANCE`), PATs still accepted for CLI clients. Claude-compatible tool names (`leads_import`, …). New workflow tools: `leads_import` (same row validation/de-dup as the web import, via `src/lib/crm/lead-import-rows.ts`), `leads_get`, `leads_convert`, `documents_request_from_client` (emails a signed upload link), `documents_create_upload_link` (public `/u/<token>` page, platform `DocumentManager`), `bills_submit_reading` (bill read by Claude → archived JSON document + VALIDATED extraction), `dossiers_activity`, `client_offers_list`, `client_offers_approve`, `pipeline_summary`. `client_offers_generate_pdf` returns 72 h signed download links; `client_offers_send` uses Cloudflare Email and records `QUEUED` like the platform. Secrets: `LINK_SECRET`, `CLERK_SECRET_KEY`, `GCP_WIF_PRIVATE_KEY`.
