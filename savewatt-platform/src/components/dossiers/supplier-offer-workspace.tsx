...
import type { SupplierOfferRecord } from "@/lib/offers/offer-types";
import { Button } from "@/components/ui/button";
import { Field, Input, Select } from "@/components/ui/field";

const cadrans: Cadran[] = ["HPH", "HCH", "HPE", "HCE"];

export function SupplierOfferWorkspace({
  dossierId,
  initialOffer,
}: {
  dossierId: string;
  initialOffer: SupplierOfferRecord | null;
}) {
  const t = useTranslations("supplierOffer");
  const router = useRouter();
  const [cee, setCee] = useState(String(initialOffer?.ceeEurMwh ?? 3));
  const [capacity, setCapacity] = useState(String(initialOffer?.capacityEurMwh ?? 2));
  const [subscription, setSubscription] = useState(
    String(initialOffer?.subscriptionEurMonth ?? 30),
  );
  const [validUntil, setValidUntil] = useState(initialOffer?.validUntil ?? "");
  const [termYears, setTermYears] = useState(String(initialOffer?.termYears ?? 3));
  ...
  function updateLine(index: number, key: "electronEurMwh" | "annualVolumeMwh", value: string) {
    setLines((current) =>
      current.map((line, lineIndex) =>
        lineIndex === index ? { ...line, [key]: Number(value.replace(",", ".")) } : line,
      ),
    );
  }

  async function save() {
    const numeric = [cee, capacity, subscription, termYears].map((value) =>
      Number(value.replace(",", ".")),
    );
    if (
      numeric.some((value) => !Number.isFinite(value) || value < 0) ||
      lines.some(
        (line) => !Number.isFinite(line.electronEurMwh) || !Number.isFinite(line.annualVolumeMwh),
      )
    ) {
    ...
    setError("");
    try {
      const response = await fetch(`/api/crm/dossiers/${dossierId}/supplier-offer`, {
        method: "POST",
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_18rem]">
        <section className="rounded-2xl border border-line bg-surface p-5">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label={t("ceePrice")}>
              <Input inputMode="decimal" value={cee} onChange={(e) => setCee(e.target.value)} />
            </Field>
            ...
            <Field label={t("subscriptionPrice")}>
              <Input inputMode="decimal" value={subscription} onChange={(e) => setSubscription(e.target.value)} />
            </Field>
          </div>
            ...
            <table className="w-full min-w-[520px] text-sm">
              ...
              <tbody className="divide-y divide-line">
                {lines.map((line, index) => (
                  <tr key={line.cadran}>
                    <td className="px-4 py-3 font-mono font-semibold text-ink">{line.cadran}</td>
                    <td className="px-4 py-2">
                      <Input
                        className="ml-auto max-w-36 text-right font-mono"
                        value={String(line.electronEurMwh)}
                        onChange={(e) => updateLine(index, "electronEurMwh", e.target.value)}
                      />
                    </td>
                    <td className="px-4 py-2">
                      <Input
                        ...
                        onChange={(e) => updateLine(index, "annualVolumeMwh", e.target.value)}
                      />
                    </td>
                  </tr>
                ))}
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-line bg-surface p-5">
            ...
            <h2 className="mt-3 text-sm font-semibold text-ink">{t("csvImport")}</h2>
            <p className="mt-1 text-xs leading-5 text-muted">{t("csvDisabled")}</p>
